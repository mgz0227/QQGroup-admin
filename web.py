...
class GroupAdminWeb:
    ...
    def _validated_save(cls, payload: Any) -> dict[str, Any]:
        ...
        mode = cls._text(payload.get("mode"), "审核方式", 16, required=True)
        if mode not in {"off", "uid", "native"}:
            raise ValueError("审核方式只能是 off、uid 或 native")
        ...
        keyword_text = cls._text(
            payload.get("uid_reject_keywords", ""),
            "拒绝关键词",
            7_000,
            multiline=True,
        )
        ...
        return {
            ...
            "uid_reject_keywords": "\n".join(parse_keywords(keyword_text)),
