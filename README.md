...
“运行配置”可设置最高优先级的全局拒绝关键词、审核设置面板是否在 45 秒后自动撤回、禁言成功回复和各轮询间隔。全局 AI 内容审核也在这里配置：选择一个主模型和最多 3 个有序回退模型；仅当上一个模型超时、报错或返回无效结果时才继续尝试，总超时可设置为 5-120 秒。AI 默认要求模型返回带置信度的 `BLOCK` 且达到阈值（默认 95）才处理，并默认“只记录、不撤回”；确认模型稳定后可切换为“撤回消息”。普通聊天、轻度玩笑和游戏/动漫截图默认放行；是否把图片送给视觉模型可单独开启。图片 OCR 关键词审核默认关闭，可配置全局关键词、OCR 模型、单条图片数和超时。全局文字关键词和全局图片关键词各自拥有独立的撤回回复与艾特开关；回复模板留空且关闭艾特时不发送提示。禁言回复支持 `{at_user}`、`{duration}`、`{expire_at}`、`{member_openid}` 变量；将 `{at_user}` 放在模板任意位置即可使用 QQ 原生艾特，不填写则不艾特。
...
- AI 审核只处理未命中本地规则的消息；主模型仅在异常、超时或输出不是带置信度的 `ALLOW`/`BLOCK` 时按顺序调用全局回退模型，总超时会为剩余模型预留预算，全部失败时放行并限频记录，避免模型故障阻塞整群。默认命中只写入违规记录，不撤回、不计入 UID 违规次数；切换为撤回动作后才执行处置。
