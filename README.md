...
通用插件配置页的“全局关键词回复”支持包含关键词和完全匹配两种方式。每条规则可填写多个群 OpenID，使用逗号、空格或换行分隔；留空时覆盖机器人能收到消息的全部 QQ 群。单群规则在“QQ群审核管理”中编辑并优先于全局规则，同一条消息只执行首个匹配项。关键词回复不要求开启消息审查，但 QQ 开放平台仍需开启“接收所有群消息”，否则机器人只能响应 @机器人的消息。
