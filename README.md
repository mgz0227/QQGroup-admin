...
首次成功轮询只建立状态，不补发历史动态或当前直播。后续会推送新动态、开播和下播；消息优先使用 AstrBot 内置 HTML 转图生成 B 站图片卡片，按作者、类型、时间、正文和封面分层，并在同一条富媒体消息中保留原动态 URL。渲染、图片上传或权限不可用时自动回退为 Markdown，再失败则发送纯文本命名链接。同一 UID 被多个群订阅时只请求一次再分发；接口返回的占位横线会被清理，图文和转发动态会尽量提取嵌套封面。网络失败、限流或风控不会清空游标，也不会误报下播。
