...
首次成功轮询只建立状态，不补发历史动态或当前直播。后续会推送新动态、开播和下播；消息采用“事件标题 + UP 主/类型/时间 + 摘要 + 查看原动态/进入直播间”排版，同一 UID 被多个群订阅时只请求一次再分发。网络失败、限流或风控不会清空游标，也不会误报下播；Markdown 不可用时降级为命名链接纯文本。
