...
WebUI 保存的是期望状态。绑定后修改条件会在下个周期生效；切换 QQ 白名单策略需点击“应用”。插件隐藏记录策略 ID、平台 ID 和上次成功应用的白名单，后续只增删名单差异。通用插件配置页还可自定义禁言成功回复，并决定是否艾特被禁言成员。
...
条件审核会读取用户主动入群申请的验证内容。硬拒绝关键词始终优先；B 站 UID 与通过关键词可按“全部满足”或“任一满足”组合，条件未通过时可选择保留待审、拒绝或同意。UID 接受 `UID:188144093`、`UID：188144093` 或纯数字格式，并通过 B 站用户名片接口核对返回值；B 站限流、网络错误或异常返回始终保留待审，避免误审。机器人邀请产生的申请不自动处理，可用 `/申请列表` 的按钮审核。
