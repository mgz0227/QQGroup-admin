...
- AI 审核只处理未命中本地规则的消息；主模型仅在异常、超时或输出不是带置信度的 `ALLOW`/`BLOCK` 时按顺序调用全局回退模型，全部失败时放行并限频记录，避免模型故障阻塞整群。默认命中只写入违规记录，不撤回、不计入 UID 违规次数；切换为撤回动作后才执行处置。
- 连续发图按成员和群级跨成员纯媒体链分别统计，达到数量、时间窗和人数条件后最多撤回配置数量的消息并发出警告；普通文本会打断连续链，不会把不同图片当作相同复读内容。
- 图片文字审核只有在全局 OCR 开关开启且配置关键词时才运行；本地 OCR/视觉 OCR 任一失败均不阻断消息，避免网络或模型异常造成误撤回。
