...
首次成功轮询只建立状态，不补发历史动态或当前直播。后续会推送新动态、开播和下播；消息采用“状态标题 + 封面 + UP 主/类型/时间 + 可选标题/摘要 + 查看原动态”的克制卡片排版，只有接口返回真实正文时才显示对应内容。同一 UID 被多个群订阅时只请求一次再分发；接口返回的占位横线会被清理，图文和转发动态会尽量提取嵌套封面。网络失败、限流或风控不会清空游标，也不会误报下播；Markdown 或图片转存不可用时降级为命名链接纯文本。
